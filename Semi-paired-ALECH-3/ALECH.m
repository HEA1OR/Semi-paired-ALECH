...
function [B1, B2, B3, B4] = ALECH(X, Y, VI,XU, YU,VU, L,LU, param, XTest, YTest, VTest)
G= NormalizeFea(L,1);
[n, dX] = size(X);
dY = size(Y,2);
X = X'; Y=Y';VI=VI'; L=L';G=G';
XU=XU';YU=YU';VU=VU';
c = size(L,1);
...
AV=pinv(SV'*(VI'*VI)*SV+delta*eye(1200))*SV'*(VI'*VU);
...
%     B1Y=[B1;BUY];
   ...
   % B2 = XTest*Gx'>0;
    ...
    GVnew=lyap(pinv(gamma*eye(nbits))*(B*B'+lamb*eye(nbits)),(VU*VU')*pinv(VI*VI'),-(pinv(gamma*eye(nbits))*(2*nbits*B*(G'*G)*VI' - nbits*B*(On'*On)*VI' +lamb*B*VI'+gamma*V*SV*AV*VU')*pinv(VI*VI')));
